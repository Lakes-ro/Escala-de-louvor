...
import {
  ...
  allClientes, loadAllClientes, formatCurrency
} from './admin.js';

let chartDonut = null;
let chartBar   = null;

// Categorias de "Custo de Sobrevivência" (essencial)
const CATEGORIAS_ESSENCIAL = [
  'habitação', 'agua', 'água', 'internet', 'energia', 'transporte público',
  'alimentação', 'saúde', 'plano de saúde', 'medicamentos', 'educação'
];
...
// ── Abre o modal de dashboard de um cliente ───────────────────
async function abrirDashboard(clienteId, nome) {
  document.getElementById('modal-dashboard-title').textContent = `📊 Dashboard: ${nome}`;

  // Reset KPIs
  document.getElementById('kpi-sobrevivencia').textContent = '...';
  document.getElementById('kpi-estilo').textContent        = '...';
  document.getElementById('kpi-passiva').textContent       = '...';
...
function calcularEExibir(transacoes) {
  ...
  // Renda passiva (categoria "renda passiva" ou "dividendos", etc.)
  const rendaPassiva = receitas
    .filter(t => /passiva|dividend|aluguel|alugel|investimento/i.test(t.categoria || ''))
  ...
  // Estilo de vida = despesas totais - essencial
  const estiloDeVida = Math.max(0, totalDespesa - custoSobrevivencia);

  // Índice de cobertura = renda passiva / custo sobrevivência
  ...
  const topCats = Object.entries(categoriaMap)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8);
...
function renderDonut(essencial, estiloVida) {
  ...
  chartDonut = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [essencial || 0.001, estiloVida || 0.001],
        backgroundColor: ['#ff6384', '#f5d623'],
        borderWidth: 0,
        hoverOffset: 6,
      }]
    },
...
function renderBarChart(topCats) {
  ...
  chartBar = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { color: '#6b9e84', font: { size: 11 } }
        },
        tooltip: {
          callbacks: {
      ...
      scales: {
        x: {
          ticks: { color: '#3d6352', font: { size: 10 } },
          grid: { color: 'rgba(255,255,255,0.04)' }
        },
        y: {
          ticks: { color: '#6b9e84', font: { size: 11 } },
          grid: { display: false }
        }
      }
    }
