...
import {
  ...
  allClientes, loadAllClientes, formatCurrency, currentAdmin
} from './admin.js';
...
async function renderMetas() {
  ...
  if (error) {
    showToast('Erro ao carregar metas: ' + error.message, 'error');
    return;
  }
  ...
  grid.querySelectorAll('.btn-card.editar').forEach(btn => {
    btn.addEventListener('click', async () => {
      const { data: m } = await supabase.from('metas').select('*').eq('id', btn.dataset.id).single();
...
async function salvarMeta() {
  ...
  let error;
  if (id) {
    ({ error } = await supabase.from('metas').update(payload).eq('id', id));
  } else {
